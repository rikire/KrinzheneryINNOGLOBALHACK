...
def group_by_extensions(unique_filenames: List[str]) -> Dict[str, List[str]]:
    ...
    -------
    Dict[str, List[str]]
        Словарь группированных файлов по расширениям
...
def get_git_log_changes(author_name, file_path):
    """
    Получение изменений по указанному файлу, сделанных разработчиком

    Parameters
    ----------
    author_name : str
        Имя или почта разработчика
    file_path : str_
        Путь до файла
...
def checkout_technologies_with_llama(prompt):
    """
    Функциия, получающая запрос от пользовател и передающая его llama
...
def get_hardskills_from_llama(repo_path: str, author_name: str) -> UserCompetencyProfile:
    ...
    for i, (_, filenames) in enumerate(groups.items()):
        ...
        prompt = (
            "Проанализируй текст изменений коммита и извлеки используемые технологии, библиотеки, языки и инструменты. "
            "Оцени уровень владения каждой технологией по шкале от 1 до 10 (1 - минимальный, 10 - максимальный) на основе "
            "читаемости кода, наличия документации, проявленной опытности, сложности изменений и использования продвинутых функций. "
            "Отнеси каждую технологию к соответствующей области, такой как frontend, backend, data_science, devops, qa, embedded, gamedev, mobile или other."
        )
    ...
    resume_prompt = ("Создай краткое резюме о квалификации разработчика на основе его компетенций. "
                     "Охарактеризуй основные сильные и слабые стороны в 2-3 предложениях, избегая числовых оценок.\n")
